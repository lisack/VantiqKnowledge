...
* **customTools** (`Code Block`) -- A code block used to define custom tools as Pydantic model classes. When the LLM selects a custom tool, the execution mode always defaults to `Return Schema Mapping`. When a single LLM response mixes custom tools with executable tools (procedures, services, or MCP tools), the executable tools run for their side effects and the custom tool calls are returned without re-invoking the LLM (see [Tool Custom Schema](llms.md#tool-custom-schema)).
