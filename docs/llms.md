...
Vantiq integrates with AWS Bedrock, allowing access to its hosted models. A list of available
models can be found in the IDE LLM creation pane, in the Model Name drop-down list.

When you choose a Bedrock model from the Model Name list, the user interface automatically 
...
**Mixing executable and custom-schema tools.** An LLM can be configured with both executable tools (VAIL procedures or MCP tools, which the Vantiq server can invoke on the LLM's behalf) and custom-schema tools (which are returned to your application). If, in a single response, the LLM selects a mix of both kinds (parallel tool calling), the Vantiq server invokes all of the executable tools and then returns only the custom-schema _tool call_ description(s) to your application, without re-invoking the LLM. Because there is no re-invocation, the values returned by the executable tools are **not** added to the LLM context — in this case those tools run for their side effects only. This is expected: a model that requests tools in parallel treats them as independent, so a custom-schema tool returned alongside an executable tool does not rely on that tool's result. When a custom-schema tool's input *does* depend on an executable tool's output, the model requests them sequentially instead — the executable tool runs, the LLM is re-invoked with its result, and the custom-schema tool is selected on that later turn.
